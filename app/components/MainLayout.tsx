            ...
            {menuItems.map((item) => {
              ...
              return (
                <Link
                  ...
                  href={item.path}
                  className={`
                    group flex items-center gap-3 p-2 rounded relative overflow-hidden transition-all duration-300
                    ${
                      isActive
                        ? 'bg-[#F9941F] text-white'
                        : 'text-[#123859] hover:text-[#F9941F]'
                    }
                  `}
                >
                  {/* Highlight animado */}
                  <span
                    className={`absolute inset-0 bg-[#F9941F] opacity-0 group-hover:opacity-10 transition-opacity duration-300 rounded`}
                  />
                  <span
                    className={`relative transition-colors duration-200 ${
                      isActive ? 'text-white' : 'text-[#123859] group-hover:text-[#F9941F]'
                    }`}
                  >
                    {item.icon}
                  </span>
                  <span className={`relative ${!sidebarOpen ? 'hidden' : 'transition-colors duration-200'}`}>
                    {item.label}
                  </span>
                </Link>
              );
