...
export default function CadastroPage() {
  ...
  // refs para gerir previews atuais e garantir cleanup correto no unmount
  const companyLogoPreviewRef = useRef(null);
  const avatarPreviewsRef = useRef({});

  // ID incremental previsível (evita usar Date.now() diretamente)
  ...
  useEffect(() => {
    // cleanup no unmount: revoga quaisquer object URLs que estejam ainda registados nas refs
    return () => {
      if (companyLogoPreviewRef.current) {
        try { URL.revokeObjectURL(companyLogoPreviewRef.current); } catch (e) {}
  ...
  function handleLogoUpload(file) {
    ...
    // revoga preview anterior (se existir) antes de criar novo
    if (companyLogoPreviewRef.current) {
      try { URL.revokeObjectURL(companyLogoPreviewRef.current); } catch (e) {}
    }
    ...
    setUsers((list) => {
      return list.map((u) => {
        ...
        if (!file) {
          ...
          if (avatarPreviewsRef.current[id]) {
            try { URL.revokeObjectURL(avatarPreviewsRef.current[id]); } catch (e) {}
            delete avatarPreviewsRef.current[id];
          }
          return { ...u, avatarFile: null, avatarPreview: null };
        }
        if (file.size > MAX_FILE_SIZE) {
          setError(`avatar_${id}`, `Ficheiro demasiado grande. Máx ${Math.round(MAX_FILE_SIZE / 1024 / 1024)}MB.`);
          return u;
        }
        clearError(`avatar_${id}`);

        // revoga preview anterior para este avatar (se existir)
        if (avatarPreviewsRef.current[id]) {
          try { URL.revokeObjectURL(avatarPreviewsRef.current[id]); } catch (e) {}
        }
        const preview = URL.createObjectURL(file);
  ...
  function validateStep1() {
    ...
    else if (!isNIF(company.nif.trim())) e.nif = "NIF inválido (apenas dígitos).";
  ...
  function togglePayment(key) {
    setCompany((c) => ({ ...c, paymentMethods: { ...c.paymentMethods, [key]: !c.paymentMethods[key] } }));
  }

  function addUser() {
    const id = nextUserId.current++;
    setUsers((u) => [...u, { id, name: "", email: "", role: "Financeiro", avatarFile: null, avatarPreview: null }]);
  }
  function removeUser(id) {
    setUsers((u) => {
      const toRemove = u.find((x) => x.id === id);
      if (toRemove && toRemove.avatarPreview) {
        // revoga preview (já guardado em avatarPreviewsRef)
        if (avatarPreviewsRef.current[id]) {
          try { URL.revokeObjectURL(avatarPreviewsRef.current[id]); } catch (e) {}
          delete avatarPreviewsRef.current[id];
        }
  ...
  async function submit() {
    ...
    try {
      ...
      Object.entries(company.paymentMethods).forEach(([key, value]) => {
        formData.append(`paymentMethods[${key}]`, value ? "1" : "0");
      });
      ...
      // armazenar token/role apenas no cliente (isto já corre no cliente)
      try {
      ...
      setSuccessMessage("Cadastro feito com sucesso! Redirecionando...");
          ...
          {[1,2,3].map((s) => (
            <div key={s} className="flex-1">
              <div className={`w-8 h-8 mx-auto rounded-full flex items-center justify-center mb-2 text-white font-bold
                ${step === s ? 'bg-[#F9941F]' : step > s ? 'bg-green-500' : 'bg-gray-300'}`}>
        ...
        {step === 1 && (
          <div className="space-y-4 ">
            ...
            <div>
              ...
              <input type="file" ref={logoInputRef} onChange={(e)=>handleLogoUpload(e.target.files ? e.target.files[0] : null)} />
            ...
            {users.map((u, idx)=>(
              ...
              <div key={u.id} className="border p-4 rounded relative">
                ...
                <div>
                  ...
                  <input type="file" ref={el=>avatarInputRefs.current[u.id]=el} onChange={(e)=>handleAvatarUpload(u.id, e.target.files ? e.target.files[0] : null)} />
